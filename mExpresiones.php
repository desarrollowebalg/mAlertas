...
**/
	...
	$sql_eventos = "SELECT EV.COD_EVENT,EV.DESCRIPTION FROM ADM_EVENTOS EV 
					INNER JOIN ADM_EVENTOS_EQUIPOS EE ON EV.COD_EVENT = EE.COD_EVENT_DEFAULT
					INNER JOIN ADM_EQUIPOS AEQ ON EE.COD_TYPE_EQUIPMENT = AEQ.COD_TYPE_EQUIPMENT
					INNER JOIN ADM_UNIDADES_EQUIPOS AUE ON AEQ.COD_EQUIPMENT = AUE.COD_EQUIPMENT
					INNER JOIN ADM_USUARIOS_GRUPOS AUG ON AUG.COD_ENTITY =  AUE.COD_ENTITY
					WHERE EE.COD_EVENT_DEFAULT <>0 AND AUG.ID_USUARIO =".$idUsuario."
					GROUP BY EV.COD_EVENT ORDER BY EV.DESCRIPTION";
